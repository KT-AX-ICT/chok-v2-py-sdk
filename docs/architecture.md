...
RCA 시스템의 실서비스 측 엣지 수집기. 30초 주기로 원천 데이터를 관측하다가 baseline 대비 이상
트리거가 발화하면 스냅샷 번들을 중앙 서버로 전송한다. **정상 구간은 전송하지 않는다.**
...
| ④ 트리거 | `trigger/` | baseline 편차 감지 + 모달리티 상관 |
...
`AnoMod/analysis/sn_db/detectors` 의 evaluate/correlate 순수 로직을 포팅한다. 단, 폴더 전체를
읽는 배치형 `detect()` 는 실시간 버퍼 입력으로 재작성한다.
