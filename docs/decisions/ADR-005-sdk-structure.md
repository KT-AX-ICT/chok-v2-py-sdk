...
```
Collector ──RawBatch──▶ Normalizer ──NormalizedBatch──▶ MemoryBuffer
                                                             │ get_snapshot(start,end)
   TriggerDetector.evaluate(new_batch, buffer)              ▼
        └── list[TriggerEvidence] ──▶ SnapshotManager ──SnapshotBundle──▶ Transport ──▶ SubmissionResult

runtime.runner 가 위 전부를 30초 루프로 오케스트레이션한다.
```
