...
### 0-6. 재생 충실도 기준 (Phase 6 검증용)

원본에서 세어둔 값이다. 재생 산출물에서 같은 값이 나오면 충실하게 옮겨진 것이다 — 세어서 대조할 수
있는 앵커로 쓴다.

| 시나리오 | 앵커 | 원본 실측 |
|---|---|---|
| cpu | `system_cpu_usage` 최대 | 100.00 (95 이상 19/80 샘플) |
| kill_media | `MediaService_.log` 의 "Starting the media-service server" | 2건 — `00:01:57.490`, `00:03:41.500` (간격 104초) |
| code_media | `all_traces.csv` 의 `http_status_code=500` | 70건 / 전체 1595 |
| code_media | `NginxThrift_.log` 의 `Could not resolve host for client socket.` | 200건 |
| 전 시나리오 | `UserService_.log` 의 `<error>` | 200건 (3종 동일 — 워크로드 배경 노이즈) |

### 0-7. 결함 시점 — `--duration` 산정 근거

| 시나리오 | 데이터 전체 길이 | 결함 시점 | 근거 |
|---|---|---|---|
| cpu | 1200s | +105s | `system_cpu_usage` 첫 95 돌파 |
| kill_media | 1187s | +107s | "Starting" 2회차 |
| code_media | 1514s | +124s | 첫 `<error>` / nginx 첫 행 |

세 시나리오 모두 결함이 **105~124초**에 온다. 뒤 15~20분은 결함 후 데이터가 이어질 뿐이다.
...
| # | 질문 | 결정 |
|---|---|---|
| D1 | `var/<service>.jsonl` 의 service 표기형 | **`media-service`** — 정확성엔 영향 없다(아래). 3종 중 2종이 이미 그 표기라 log 만 매핑하면 되고, 디스크에서 일관되게 보인다 |
...
파일명이 달라도 소비하는 쪽이 흡수한다. 일관성 문제일 뿐이니 `media-service` 로 통일하고 넘어간다.
...
`/speed` 를 안 하면(간격 보존) 타임스탬프가 벽시계보다 미래로 달려나간다. 그런데 `/speed` 를 하면
**간격이 배속만큼 줄어든다** — "Starting" 2회 간격이 1x 에서 104초, 10x 에서 10.4초다. 즉 배속을 켜는
순간 **재생 산출물이 원본과 달라진다.** 이 계획의 완료 기준(원본 충실도) 자체를 무너뜨린다.

**대신 `--duration` 으로 데모 길이를 줄인다.** 배속이 풀려던 문제는 "20~25분을 다 볼 수 없다"였고,
이건 끊는 것으로 해결된다 — 타임스탬프를 건드리지 않는다. 0-7 기준 결함이 105~124초에 오므로
**5분이면 3종 전부 결함을 덮는다.**
...
- **상대 간격을 보존한다.** "Starting" 2회 간격 104초가 재생 후에도 104초여야 한다. 배속이 없으므로
  (D5) 나눗셈이 없고 타임스탬프가 벽시계를 그대로 따라간다.
...
- [ ] **0-6 의 앵커가 재생 산출물에 같은 수로 남아 있다**:
  - `var/metric/` 에 value ≥ 95 인 레코드 (cpu)
  - `var/log/media-service.jsonl` 에 "Starting the media-service server" 2건, 간격 104초 (kill_media)
  - `var/trace/` 에 `http_status_code=500` 70건 (code_media)
  - `var/log/nginx-thrift.jsonl` 에 "Could not resolve host" 200건 (code_media)
  - `var/log/user-service.jsonl` 에 `<error>` 200건 (3종 전부)
- [ ] 모든 줄이 `json.loads()` 가능
- [ ] 안티패턴 grep:
