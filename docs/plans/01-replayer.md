...
- 병합은 파일별 스트림의 k-way merge 다 — 각 스트림 안의 순서는 절대 바꾸지 않고, 다음에 내보낼
  스트림만 고른다. 파일 단위로 순차 처리하면 시각축이 깨진다. `heapq.merge(key=ts)` 로 구현한다 —
  정렬 안 된 로그 스트림에서도 각 스트림을 준 순서대로 전진시키며 head 가 이른 것을 고른다.
- **`--duration` 은 재생 시간축 기준이다** — `new_ts` 가 `anchor + duration` 을 넘으면 멈춘다.
  페이싱이 `new_ts` 를 현재 시각에 맞추므로 실측 경과 시간과 같고, 테스트는 clock 주입으로 재우지 않는다.
- **0바이트 소스는 출력 파일을 만들지 않는다.** cpu/kill_media 의 `NginxThrift_.log` 가 그렇다 —
  원본 0줄 == 출력 0줄이라 충실하다. 존재하되 비어 있는 것과 부재의 차이는 콜렉터의 파일 부재 처리
  몫이지 리플레이어 몫이 아니다.
- **t0 는 로그를 두 번 스트리밍한다** — 한 번은 min 측정, 한 번은 재생. CSV 는 정렬하며 적재해 첫 행이
  min 이고 재사용한다. 로그(최대 27MB)는 적재하지 않는다 (함정 5).
...
- [x] `uv run python -m demo.replayer cpu --reset` 후 `var/{log,metric,trace}/` 에 원본과 같은 이름의 파일 생성
- [x] `uv run python -m demo.replayer --help` 가 3종 시나리오와 `--duration` 을 보여준다
- [x] 잘못된 CWD 에서 실행 시 절대경로+CWD 를 담은 명확한 실패
- [x] `--duration` 생략 시 전체 재생, 지정 시 그 시각에 정지한다 — 데이터 끝에서 반복하지 않고 정지
- [x] `cpu --duration 30` 후 `code_media --duration 30` 을 이어 돌리면 `runs.csv` 에 두 행이 남고,
...
- [x] 3종 전부 재생 성공: `for s in cpu kill_media code_media; do uv run python -m demo.replayer $s --reset; done`
- [x] **누락 없음** — `var/` 각 파일의 줄 수 == 원본 파일의 줄 수
- [x] **내용 보존** — 각 줄이 원본과 타임스탬프 외 바이트 동일
- [x] **간격 보존** — 인접 타임스탬프 차이가 원본과 동일
- [x] `var/` 파일 목록이 원본의 재생 대상 목록과 일치 (log 11~12 / metric 15 / trace 1)
- [x] 안티패턴 grep:
...
- [x] `uv run pytest` / `uv run ruff check .` 통과
- [x] `git status` 에 `var/` 가 안 잡힌다
