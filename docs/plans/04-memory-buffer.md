...
3. **창 기반 detector 가 이미 번들로 보낸 구간을 다시 센다** — **미해결.** 번들 종료 후 재발화해 내용이
...
5. **`cpu_spike` 판정 규칙이 근거와 불일치** — **미해결.** upstream 이 `metric_name` 은 고쳤으나
   ADR-006 의 "baseline 3/79 **산발**" 서술은 4곳 모두 그대로다(§51·66·74·90). 아래 §8.

## §8. `cpu_spike` 판정 규칙 제안 (설계만, 코드 미변경 · 미해결)
