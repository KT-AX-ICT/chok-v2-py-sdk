...
1. **`cpu_spike` 가 발화 불가** — detector 는 `metric_name == "system_cpu"` 를 찾는데(ADR-006 §조건),
   정규화는 파일명 stem 을 쓰므로 `system_cpu_usage.csv` → `"system_cpu_usage"` 가 된다.
   이름이 안 맞아 초과 샘플이 항상 0건 → Perf CPU 트리거가 성립하지 않는다. **최우선.**
2. **`modality_info` 가 missing 을 못 낸다** — `_modality_info` 는 `record_count` 만 합산해
   `data`/`empty` 만 낸다. ADR-007 §6 이 "missing 은 Normalizer roster 완성 후"로 유보한 항목인데,
   roster 는 이제 `present` 를 채우므로 해소 가능하다. Code_Stop 국소화의 부재 신호가 여기 걸린다.
3. **창 기반 detector 가 이미 번들로 보낸 구간을 다시 센다** — 번들 종료 후 재발화해 내용이
   ...
   **제안**: 되돌아보기 시작점을 직전 번들 이후로 자른다.

   ```
   lookback_start = max(observed_until − window_sec, 직전 번들 window_end)
   ```

   그러면 번들 이후 데이터만으로 다시 세므로 "임계를 다시 넘으면 그 시점 앞뒤 3분" 이라는
   의도대로 동작하고, k번째 샘플이 곧 최신이라 **anchor 가 저절로 최신**이 되어 pre 유실도
   같이 사라진다. anchor 를 배치 경계로 옮기는 것은 이 문제에 대해선 불필요해진다.

   구현 비용: detector 가 "직전 번들 종료 시각"을 알아야 하므로 `evaluate()` 시그니처
   (계약 §2.4) 또는 Runner 배선에 전달 경로가 필요하다 — trigger 계층 결정 사항.
4. **trigger·snapshot 단위 테스트 부재** — ADR-007 이 "단위 9 테스트(FakeBuffer) 통과"를 기록했으나
   `tests/` 에 해당 파일이 없다(main·feat/snapshot-manager·feat/modality-trigger-detectors 모두).

5. **`cpu_spike` 판정 규칙이 근거와 불일치** — 아래 §8.

## §8. `cpu_spike` 판정 규칙 제안 (설계만, 코드 미변경)
