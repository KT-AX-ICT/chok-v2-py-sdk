...
3. **재발화 시 pre 가 조용히 잘린다** — 번들 종료 후 같은 이유로 재발화해 창이 겹치는 번들을
   또 보내는 것은 **설계 의도**다(중복 허용, 쿨다운 불필요). 문제는 그때 anchor 다: 창 기반
   detector 는 `trigger_time` 을 창 안의 과거 레코드에서 고르므로, 재발화 시 anchor 가
   watermark 보다 100초 이상 낡을 수 있다. 그러면 `pre = [anchor−180, anchor)` 가 보존
   구간(210초)을 벗어나 **앞부분이 경고 없이 빠진다**. anchor 를 발화 배치의 `observed_until`
   로 잡으면(배치 정렬) 구조적으로 해소된다 — 보존을 키우는 것은 대증요법.
