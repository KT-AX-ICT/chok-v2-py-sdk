...
class SnapshotManager:
    ...
    def _assemble(self, session: _CaptureSession, post: MultimodalSnapshot) -> SnapshotBundle:
        ...
        metrics = [*pre.metrics, *post.metrics]
        traces = [*pre.traces, *post.traces]

        if self._log_truncation_enabled:
            # trigger 근거가 지목한 서비스는 절대 안 자른다 (2026-07-23 설계) —
            # restart_marker 는 재시작 서비스, nginx_error/trace_5xx 는 nginx 처럼
            # detector 가 직접 귀속시킨 서비스.
            exempt = {e.service for e in session.evidences if e.service}
        ...
        return SnapshotBundle(
            ...
            logs=[_rec(r) for r in logs],
            metrics=[_rec(r) for r in metrics],
            traces=[_rec(r) for r in traces],
            modality_info=_modality_info(
                pre, post, session.window_start, session.window_end, logs, metrics, traces
            ),
        )


def _is_protected(record: NormalizedLog, exempt_services: set[str | None]) -> bool:
    # error/warn 등 info 아닌 레벨, service_start/connection_error 등 normal_log 아닌 이벤트,
    # trigger 근거가 지목한 서비스 — 이 중 하나라도 해당하면 truncate 대상에서 제외한다.
    return (
        record.service in exempt_services
        or record.level != "info"
        or record.event_type != "normal_log"
    )


def _stride_sample(records: list[NormalizedLog], n: int) -> list[NormalizedLog]:
    """균등 간격 샘플링 — head-N 대신 창 전체 시간대에 걸친 모양을 유지한다.

    `records` 는 시간순 정렬 전제(호출부가 보장). 정확히 min(len(records), n)개를 반환한다.
    """
    if n <= 0:
        return []
    if len(records) <= n:
...
def _truncate_logs(
    records: list[NormalizedLog],
    exempt_services: set[str | None],
    cap: int,
    backstop_cap: int,
) -> list[NormalizedLog]:
    """서비스별 볼륨 캡 (2026-07-23 설계, docs 논의 근거).

    대상: level=="info" AND event_type=="normal_log" AND service 가 trigger 비귀속인 레코드만.
    초과분은 균등 간격 샘플링으로 cap 개까지 줄인다. 그 외(보호 레코드)는 서비스별
    backstop_cap 만 최후 방어선으로 적용 — 정상 동작에서는 걸리지 않지만, trigger 귀속
    서비스 자체가 폭주하는 미지의 사태에도 번들 크기 상한을 수학적으로 보장한다.
    """
    if not records:
        return records

    by_service: dict[str | None, list[NormalizedLog]] = defaultdict(list)
    for r in records:
        by_service[r.service].append(r)

    out: list[NormalizedLog] = []
    for recs in by_service.values():
        protected = [r for r in recs if _is_protected(r, exempt_services)]
        candidates = [r for r in recs if not _is_protected(r, exempt_services)]
        combined = protected + _stride_sample(candidates, cap)
        if len(combined) > backstop_cap:
            combined.sort(key=lambda r: r.timestamp)
            combined = _stride_sample(combined, backstop_cap)
        out.extend(combined)

    out.sort(key=lambda r: r.timestamp)  # 서비스별로 합쳤으니 전체 재정렬 필요 (버퍼 계약 유지)
    return out
...
def _modality_info(
    ...
    # 이 total_count 는 roster 기반이라 truncate 와 무관하게 항상 "원래 있었던 진짜 건수"다 —
    # missing/empty/data 판정은 우리가 얼마나 실었는지가 아니라 원본 상태를 반영해야 하기 때문.
    ...
    # 번들에 실제로 담긴(= truncate 반영 후) 소스별 건수. log 만 truncate 대상이라 의미가
    # 있고, metric/trace 는 원본과 항상 같다. truncate 여부는 record_count < total_count 로
    # 서버가 직접 판별한다 — 별도 bool 필드는 서버 요청으로 뺐다 (2026-07-23).
    included: Counter[tuple[str, str]] = Counter()
    for modality, records in (("log", logs), ("metric", metrics), ("trace", traces)):
        for r in records:
            if r.service:
                included[(modality, r.service)] += 1

    info: dict[str, ModalityInfo] = {}
    for (modality, source), (present, total_count) in merged.items():
        record_count = included.get((modality, source), 0)
        if total_count > 0:
            status_value = "data"
        elif present:
